...
| T-5  | T      | TOFU host-key acceptance (`AutoAddPolicy`) — MITM at first registration silently owns the channel | A-1, A-2 | U-2 | **Closed in Phase 2c CP4.4** — `KnownHostsCAPolicy` enforces that every host cert chain back to the Vault CA the worker minted its user cert against, and (since the host-cert validity fix) that the cert is inside its `valid_after`/`valid_before` window — expired certs were previously accepted — and that the cert was issued for the host being dialed (principal match), so a compromised managed host holding a CA-signed cert can no longer impersonate another. Celery beat renews host certs before expiry. |
...
| T-13 | S      | Enrollment token read from userdata / instance metadata lets an attacker enroll a rogue host onto the VPN | A-3 | U-2 | **Partially mitigated in Phase 3f MVP**: tokens are single-use by default, expire (60 s to 7 d), are tied to one hub and tenant, and can only be minted by admins. The management SSH identity is fixed at mint time. Every redemption lands in the audit table as `client.enroll`, and every failure as `enroll.reject`. A suspected leak can be cut off immediately with `POST /v1/enrollment-tokens/{id}/revoke`, which also stops a redemption already in progress. Still open: source/instance binding (Phase 3f hardening) and cloud instance-identity attestation in place of bearer tokens (Phase 3f polish). |
