...
# Production-Vault entrypoint shim. Reads ${VAULT_INIT_FILE}
# (default /app/vault-init.json — the prod compose overlay bind-
# mounts the file generated by `vault operator init` into every
# wg-manager container) and exports the root token as VAULT_TOKEN
# before exec'ing the CMD. First-boot tolerant: a missing file is
# a no-op, so the dev compose (where Vault is in -dev mode and the
# token is hard-coded in env) continues to work unchanged.
COPY --chown=wgmanager:wgmanager docker/entrypoint-wg-manager.sh /usr/local/bin/entrypoint-wg-manager.sh
RUN chmod 0755 /usr/local/bin/entrypoint-wg-manager.sh
